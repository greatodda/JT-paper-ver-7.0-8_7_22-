...
a \emph{semistandard tableau} of shape $\lambda / \mu$ is a filling of the diagram of $\lambda/\mu$ by positive integers which weakly increase across rows and strictly increase down columns.
Then $s_{\lambda/\mu}$ is the sum of monomials  $\mathbf{x}^T = {\prod\nolimits_{i}} \, x_i^{\text{\# of i's in T}}$ over all semistandard tableaux  $T$ of shape $\lambda / \mu$.
...
only need the combinatorial description of $s_{\lambda/\mu}$ in minor way in later proofs.
%
...
\begin{itemize}
\item each asterisk (``$\ast$'') is an indeterminate or element of $\Fq$;
...
\right)
\]
(where $x,y,z,w$ are three distinct indeterminates) are slant matrices of type
...
\begin{proof}
First, we sketch the argument; then, we will track it on an example.
...
the $3$-rd-to-last column of $A_{j}$ is the $3$-rd-to-last column of $A_{i}$.
...
paradiagonal is $\left(  0,0,\ldots,0\right)  $). The full matrix obtained is
$M^{\prime}$.
...
full paradiagonal, which consisted of zeroes). The strange block either
...
$M$. Inside that induction step, make a strong induction on $2i+\ell$. So let
...
We WLOG assume that the multislant matrix $M$ is strict.\footnote{Indeed,
...
that \textquotedblleft$M$ is strict\textquotedblright\ is indeed a WLOG
assumption.}
