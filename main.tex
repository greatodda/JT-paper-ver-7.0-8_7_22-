...
\textbf{Abstract.}
%note: remove \cite when pasting abstract into arxiv, but keep in the .tex file
The problem of counting the number of $\mathbb{F}_q$-valued points of a variety  has been well studied from algebro-geometric, topological, and combinatorial perspectives.  We explore a combinatorially flavored version of this problem studied by Anzis et al. \cite{Anzis18}, which is similar to work of Kontsevich \cite{Kontsevich},  Elkies \cite{Elkies}, and Haglund \cite{Haglund}.
%Anzis et al. \cite{Anzis18} investigated the problem of counting the number of zeros of the determinant of a Jacobi-Trudi matrix when the variables are taken over  $\mathbb{F}_q$
%Anzis et al. studied the probability that the determinant of a Jacobi-Trudi matrix vanishes if the variables are chosen uniformly at random from a finite field
...
%
%\emph{Anzis et al.} \cite{Anzis18} studied the following version of this problem: determine the probability that the determinant of a Jacobi-Trudi matrix vanishes if the variables are chosen uniformly at random from a finite field.
%
%We establish our result as part of a a special case of one giving the probability of determinant vanishing for a class of matrices made up of rectangular blocks of Toeplitz matrices.
%
%We further explore this problem and generalize and prove their conjectured formula for a $2-$staircase to any partition that takes the form of an arithmetic progression.
%
%Building on work of \emph{Anzis et al.} \cite{}, we study the probablility of a
%we study
%
...
% \end{pmatrix} =0 ,
% \]
% where $z_{1},z_{2},z_{3}$ are elements of $\mathbb{F}_{q}$ chosen
...
%\end{definition}
Anzis et al. \cite{Anzis18} conjectured that a $2$-shifted $2$-staircase $\lambda$ satisfies $P(s_{\lambda} \longmapsto 0) = \dfrac{q^2+q -1}{q^3}$. We prove this result and generalize it to arbitrary $n$ and $p$:
...
homomorphism $\varphi:\Lambda_{\leq N}\rightarrow\mathbb{F}_{q}$ that
sends the indeterminates $h_{1},h_{2},\ldots,h_{N}$ to $z_{1},z_{2}
,\ldots,z_{N}$, respectively. Hence the right side of \eqref{e prop eval}
...
n,\ 1\leq j\leq n}$. Applying  $\omega$ to this, 
$h_{n}$ can likewise be written as a polynomial in $e_{1},e_{2},\ldots,e_{n}$.
Thus
\begin{align}
\Lambda_{\le N} \, = &\  \big\{\text{subring of $\Lambda$ generated by $h_1, \dots, h_N$} \big\} \\
            = &\  \big\{\text{subring of $\Lambda$ generated by $e_1, \dots, e_N$} \big\},
\end{align}
...
$s_{\lambda/\mu} \in \Lambda_{\le N}$.  Since $\omega\left(  s_{\lambda/\mu}\right)
=s_{\lambda^{t} /\mu^{t}}$ (see, e.g., \cite[Theorem 7.15.6]{EC2} or \cite[(5.6)]{Macdonald}),
$\omega_N\left(  s_{\lambda/\mu}\right) =s_{\lambda^{t} /\mu^{t}}$ as well.
We thus have the following map which is a bijection since 
$\omega_N$ is an automorphism:
\begin{align}
...
& \varphi \mapsto \varphi \circ \omega_N.
\end{align}
Hence by Proposition \ref{prop.eval},
$P\left(  s_{\lambda/\mu}\mapsto a\right)  =P\left(  s_{\lambda^{t}/\mu^{t}%
}\mapsto a\right) $.
...
\begin{definition}
Let $p > 0$, $n > 0$ and $k \geq 0$ be integers.
We define the \emph{block staircase partition} to be the partition
\[
