...
\begin{align*}
J_{u_1, u_2, \ldots, u_k}\tup{x_1, x_2, \ldots, x_k}
&:=
\det \left(  x_{u_{i}-j}\right)_{1\leq i\leq k,\ 1\leq j\leq k}
\\
&= \det \begin{pmatrix}
x_{u_1 - 1} & x_{u_1 - 2} & \cdots & x_{u_1 - k} \\
x_{u_2 - 1} & x_{u_2 - 2} & \cdots & x_{u_2 - k} \\
\vdots & \vdots & \ddots & \vdots \\
x_{u_k - 1} & x_{u_k - 2} & \cdots & x_{u_k - k}
\end{pmatrix} ,
\end{align*}
where $u_{1}>u_{2}>\cdots>u_{k}>0$ are integers.
...
$\det\tup{ J_{u_1, u_2, \ldots, u_k}\tup{x_1, x_2, \ldots, x_k} }$
of this matrix vanishes.
This probability was computed in
\cite{Anzis18} for certain families of $u_i$'s.
In particular, this probability is $\dfrac{1}{q}$ in the following cases:
\begin{itemize}
\item the ``\emph{rectangle case}'': $u_i = m - i$ for each $i$ (see \cite[Corollary 6.4]{Anzis18}, or \cite{dwivedi2021rank} for a generalization);
\item the ``\emph{staircase case}'': $u_i = k - 2i$ for each $i$ (see \cite[Theorem 6.5]{Anzis18});
\item the ``\emph{hook case}'': $u_1$ arbitrary; $u_i = 1 - i$ for
each $i = 2, 3, \ldots, r$; finally, $u_i = -i$ for each $i > r$ (see \cite[Proposition 1.2]{Anzis18}).
\end{itemize}
...
In particular, \cite[Conjecture 10.1]{Anzis18} gives a formula for the probability of a determinant corresponding to a $2$-staircase vanishing. We not only prove this formula, but also generalize it to any inward-shifted $n$-staircase; we show that if the length of this $n$-staircase is $\geq n+1$, then, mysteriously, this probability equals that of a completely random generic $n\times n$-determinant vanishing.\footnote{Incidentally, this confirms the upper bound conjecture \cite[Conjecture 5.10]{Anzis18} for this class of partitions.}
In other words:
\begin{theorem}
\label{thm.n.staircase-intro}
Let $\lambda$ be a partition of the form
\[\lambda = (p+(k-1)n, \ p+(k-2)n, \ \ldots, \ p+n, \  p) , \]
where $0 < p \leq n \leq k-1$.
Then,
\[
...
We will in fact prove much more general statements that apply to a broader class of determinants with similar structures. One important result that we prove is given as follows:
\begin{theorem}
Inward-shifted staircase placeholder
\end{theorem}
...
$J_{u_1, u_2, \ldots, u_k}\tup{x_1, x_2, \ldots, x_k}$ becomes a Schur
function.
...
A question often asked is whether the number of $\Fq$-valued points has the form $f\tup{q}$ for some Laurent polynomial $f$; this is not always satisfied for $J_{u_1, u_2, \ldots, u_k}\tup{x_1, x_2, \ldots, x_k}$ (see \cite[Proposition 5.7]{Anzis18}), but is true in surprisingly many situations, including the ones we analyze here. (The same question has been asked by Kontsevich for certain graph-theoretical polynomials, and answered in the negative \cite{Belkale2003}.)

We note that the values of $\det\tup{ J_{u_1, u_2, \ldots, u_k}\tup{x_1, x_2, \ldots, x_k} }$ are not equidistributed in $\Fq$ in general, even when the value $0$ is taken with probability $\dfrac{1}{q}$. For example, equidistribution is satisfied in the hook case (\cite[Proposition 1.2]{Anzis18}) but not in the rectangle case (\cite[Lemma 9.3]{Anzis18}). Generalizing the hook case, we show that equidistribution holds for \emph{ribbons} (a class of skew partitions).
...
Example \ref{ydiagexample} below shows the construction of a Young diagram for the partition $\lambda = (7,4,1)$ and its conjugate. Note that each row has the same number of boxes as each part of the partition. Since a partition is a weakly decreasing sequence, the number of boxes in a row decreases as we go down the diagram.
...
For example, if $\lambda$ is the partition in Example \ref{ydiagexample}, then the Schur function $s_\lambda$ is given by
\begin{align*}
s_{(7,4,1)} =
\det
\begin{pmatrix}
...
Let $\lambda= (\lambda_1, \lambda_2, \ldots, \lambda_{\ell})$  and $\mu= (\mu_1, \mu_2, \ldots, \mu_{\ell}) \subseteq \lambda$ be such that $\lambda/\mu$ is a ribbon. 
...
\begin{remark}
Lemma \ref{lemma.Hessenberg} essentially claims that the subdiagonal of $J(\lambda / \mu)$ is made up of only $1$'s and the matrix has the following structure. 
\[
