...
{}
\theoremstyle{plainsl}
\ihead{On Multislant matrices and Jacobi-Trudi determinants over finite fields}
\ohead{page \thepage}
\cfoot{}
\begin{document}

\title{On Multislant matrices and Jacobi Trudi determinants over finite fields}
...
Given a partition $\lambda$ and a finite field $\F_q$,  what is the probability of an evaluation of a Jacobi-Trudi determinant corresponding to $\lambda$ vanishing? For $\lambda$ being a staircase, hook or a rectangle partition, this probability has been studied to be $\frac{1}{q}$. We investigate this probability for  skew partitions such as ribbons as well as more complex partition such as n-staircases and block staircases. In the framework of our proofs, we prove much more general statements that apply to the determinants of a broader class of matrices, which we refer to in this paper as \emph{Multislant matrices}, which are matrices of the form $\left(  A_{1}\mid
A_{2}\mid\cdots\mid A_{k}\right)  $, where $A_{1},A_{2},\ldots,A_{k}$ are
pairwise disjoint tall toeplitz matrices with the same number of rows . \\

\section{\label{sec.introduction}Introduction}

Let $\mathbb{F}_q$ be a finite field. We begin by looking at the probability that  
...
at random (uniformly \& independently). This probability is $\dfrac{1}{q}$, and is one of the main results of \cite{Anzis18} (along with similar claims for similarly structured $n\times
n$-matrices). This matrix can be seen as a special case of a Jacobi-Trudi matrix corresponding to a rectangle partition as well as particular case of a Hankel matrix. We previously provided a Hankel matrix generalization of this and similarly structured matrices in \cite{dwivedi2021rank}. In this paper we study matrices of similar kind as a particular cases of Jacobi Trudi matrices.  \\

For our Jacobi-Trudi matrix generalization, we consider  \\

$\det \left(  x_{u_{i}-j}\right) = \det \begin{pmatrix}
...
\end{pmatrix}$ \\
where
$u_{1}>u_{2}>\cdots>u_{n}>0$, with $x_{0}=1$,
$x_{i}=0$ for $i<0$ and $x_{i}$ being uniformly random over $\mathbb{F}%
_{q}$ for $i>0$. Such a
matrix is called a \emph{Jacobi--Trudi matrix}. Computing the
probability that its determinant vanishes was the main goal of
\cite{Anzis18}, which was achieved for certain families of
$u_i$'s. In particular they prove that the probability of the Jacobi-Trudi determinant vanishing is precisely $\dfrac{1}{q}$ in the following cases (see \cite[specific results]{Anzis18}):
\begin{itemize}
\item the ``\emph{rectangle case}'': $u_i = m - i$ for each $i = 1, 2,
\ldots, n$;
\item the ``\emph{staircase case}'': $u_i = n-2i+2j+1 $ for each $i,j = 1, 2,
\ldots, n$;
...
%in the case of determinants corresponding to staircase, hook and rectangle-shaped partition shapes.\\

In addition they also provide conjectures on a few more specific miscellaneous partitions. We add a few more cases and generalize and prove some of these conjectures. In particular, Conjecture 10.1 in \cite{Anzis18} states the probability of a determinant corresponding to a 2-staircase vanishing. We not only prove the 2-staircase result, but we generalize and prove the result for any arbitrary n-staircase and show that this probability, mysteriously, takes the form of that of a completely random generic $(n-1)\times (n-1)$ matrix vanishing (as discussed in the very beginning of the text). We will in fact prove much more general statements that apply to a broader class of determinants with similar structures. One important result that we prove is given as follows:
\begin{theorem}
Multislant Placeholder
\end{theorem}
Schur Polynomials provide an important motivation to study the determinants of these Jacobi-Trudi matrices, since if we replace the $x_{i}$ by the generators $h_{i}$ of the
ring of symmetric functions (see \S 2.2 for a detailed definition and further exploration), then the determinant of
the Jacobi-Trudi matrix $\left(  x_{u_{i}-j}\right)  $ becomes a Schur
function, and thus its probability of vanishing is ... %ask Darij about what did he mean to put here? Just the general description of significance of vanishing Schur polynomials or something specific intended here
This is the approach taken by \cite{Anzis18} when studying these determinants. 
Another way to look at our results could be interpreting it as counting $\mathbb{F}_{q}%
$-valued points on schemes as studied by Stanley, Weil, Kontsevich and likewise. \cite{RPStan} \cite{Hayes} \cite{Kontsevich} .
...
\section{Background and Statements of the Theorems}


\subsubsection{Mathematical Background and Schur Functions}


Fix a finite field $\mathbb{F}_{q}$. Consider the polynomial ring 

\[\mathcal{P}:=\mathbb{Z}\left[  h_{1},h_{2}%
,h_{3},\ldots\right]  \]

in countably many indeterminates $h_{1},h_{2}%
,h_{3},\ldots$.
\begin{definition}
\label{def.mapstozero}

For any $f\in\mathcal{P}$ and $a\in\mathbb{F}_{q}$, let $N\in\mathbb{N}$ such that $f$ only
involves the indeterminates $h_{1},h_{2},\ldots,h_{N}$. Define
...
This is independent on the choice of $N$
because increasing $N$ by $1$
merely multiplies the numerator and the denominator by $q$.

% \begin{remark}
% Roughly speaking, $P\left(  f\mapsto0\right)  $ is the probability that
% $f\left(  x_{1},x_{2},x_{3},\ldots\right)  =0$ when $x_{1},x_{2},x_{3},\ldots$
% are random elements of $\mathbb{F}_{q}$ (chosen uniformly at random and
% independently). (One does not need to literally choose infinitely many random
% $x_{i}$'s, since $f$ involves only finitely many $x_{i}$'s, and clearly
% choosing any more does not affect the probability.)
% \end{remark}

% $P\left(  f\mapsto a\right)$ can be similarly defined for any given $a\in \mathbb{F}_{q}.$\\

We can view the polynomial ring $\mathcal{P}=\mathbb{Z}\left[  h_{1}%
,h_{2},h_{3},\ldots\right]  $ as a ring of \textquotedblleft symmetric
functions\textquotedblright\, which are in fact symmetric bounded degree Formal Power Series, in
countably many indeterminate $\xi_{1},\xi_{2},\xi_{3},\ldots$ by identifying
each $h_{i}$ with the standard sum. For the contents of the paper, we don't actually work with the $\xi_{i}$ (in
particular, we will not substitute anything for them). However, this background is important since we will consider some standard symmetric functions as elements of this polynomial ring $\mathcal{P}$. Surveys of symmetric functions can be found in \cite{Macdonald} and \cite{EC2}.

\begin{definition}
A partition of a positive integer $n$ is a  weakly decreasing sequence of positive integers $(\lambda_1, \lambda_2, \cdots ,\lambda_k)$, with  $\lambda_1 \geq \lambda_2 \geq \cdots \lambda_k$, that sum to $n$.
\end{definition}

There are two important terms associated with a partition. 
\begin{definition}

The size of a partition is often denoted by $\ell(\lambda)$ or $|\lambda|$ and denotes the cardinality of a partition. For a partition $\lambda = (\lambda_1, \lambda_2, \cdots ,\lambda_k)$ and nonzero $\lambda_i's$, $\ell(\lambda) =k$

\end{definition}. The second term is called the conjugate (or sometimes the transpose) of a partition and is represented by $\lambda'$ or $\lambda^t$. To properly understand the meaning of the conjugate of a partition, let us first take a look at Young Diagrams.


Every partition can be represented in the form of a Young diagram which is a combinatorial object that helps us better understand the intuition of partitions. 
...
\begin{definition}
The conjugate of a  partition is defined with respect to these Young diagrams and can be determined by constructing the Young diagram of a partition and then writing down the number of boxes in each column from left to right.
\end{definition} 
...
\end{align*}
\[
\text{Figure 1. A Young diagram for $\lambda = (6,5,4,3,2,1)$}
\]

There are a few important families of partitions that will be studied in this paper so we name and define them appropriately as is done in mathematical literature. 


\begin{definition}
\begin{enumerate}
    \item A rectangle partition is a partition of the form 
\[\lambda = (a^n) = (a,a, \ldots a)\]
    \item A staircase is a partition of the form 
\[\lambda = (k, k-1, k-2, \ldots 1)\]
    \item An $n$-staircase is a partition of the form 
...
It should be easy to see that the families of partition are named after the shape of the corresponding Young diagrams of that particular family.\\

There also exists the notion of skew partitions which are a generalization of partitions to set theoretic differences of two partitions. A skew partition is a pair of partitions $(\lambda, \mu)$ such that the Young diagram of $\lambda$ contains the Young diagram of $\mu$; it is denoted by $\lambda / \mu$. If $\lambda= (\lambda_1, \lambda_2, \cdots)$  and $\mu =(\mu_1, \mu_2, \cdots)$, then the containment of diagrams means that $\mu_i \leq \lambda_i$ for all $i$. In this paper, two partitions which exhibit this property will be represented by $\mu \subseteq \lambda$. The skew diagram of a skew partition $\lambda / \mu$ is the set-theoretic difference of the Young diagrams of $\lambda$ and $\mu$: the set of boxes that belong to the diagram of $\lambda$ but not to that of $\mu$. 


We now state the Jacobi-Trudi Identity for different partitions.  The Jacobi-Trudi identity allows an easy method to calculate the Schur polynomials in term of elementary symmetric and complete homogeneous symmetric polynomials. \cite{Anzis18}
\begin{definition}[Jacobi-Trudi Identity]
\label{Jacobi-Trudi Identity}
Let $\lambda = (\lambda_1, \lambda_2, \ldots, \lambda_k)$ and $\mu = (\mu_1, \mu_2, \ldots, \mu_k) \subseteq \lambda$. Define \emph{Jacobi-Trudi matrix} of $\lambda/\mu$ to be the  $k \times k$ matrix 

\[
J(\lambda/\mu) = \left[ h_{\lambda_i-\mu_j - i + j} \right]^k_{i,j=1}.
\]
where $h_0 = 1$ and $h_m = 0$ for all $m < 0$. \\

The \emph{skew Schur function} of $\lambda/\mu$ is defined to be \begin{align*}
   s_{\lambda/\mu} = \det(J(\lambda/\mu)) \  \in \mathcal{P}
\end{align*}

When $\lambda/\mu = \lambda/\varnothing$ is a straight partition, we write $J(\lambda)$ in place of  $J(\lambda/\mu)$ and $s_{\lambda}$ in place of $s_{\lambda/\mu}$. 
...
\begin{remark}
\label{flushremark}
If $\mathcal{P}$ is viewed as the ring of the symmetric functions, then the skew Schur function $s_{\lambda/\mu}$ is often defined as a sum of monomials, one corresponding to each filling of the diagram of $\lambda/\mu$ by positive integers which weakly increase down the rows and strictly increase down the columns. With this definition, then %cite s_\lambda
 is a theorem. However, we will only need this connection to symmetric functions in a minor way and take the % same citation as above
 as the definition of $s_{\lambda/\mu}$.
\end{remark}

An example computation can help elucidate this identity. Consider the partition in Figure 1. Using the Jacobi-Trudi Identity, we can write the corresponding Schur Polynomial using the determinant 
...
We now give the statements of the general results that will be discussed in this paper. \\

We first begin with multislant matrices. 
...
\begin{itemize}
\item each asterisk (\textquotedblleft$\ast$\textquotedblright) is an element
of $F$;
...
columns of a block from the right. That is, the \textquotedblleft
corresponding column\textquotedblright\ of the $3$-rd-to-last column of
$A_{j}$ is the $3$-rd-to-last column of $A_{i}$. The reason why this
\textquotedblleft corresponding column\textquotedblright\ always exists is
...
\textquotedblleft quasi-indeterminates\textquotedblright\ (i.e., differences
of the form \textquotedblleft indeterminate minus a scalar\textquotedblright%
\ or \textquotedblleft indeterminate minus a scalar multiple of another
indeterminate\textquotedblright). However, these \textquotedblleft
quasi-indeterminates\textquotedblright\ are still uniformly independently
...
type 1. Let us refer to the latter block as the \textquotedblleft strange
block\textquotedblright. Let $M^{\prime}$ be the matrix obtained from $M$ by
...
the \textquotedblleft strange block\textquotedblright. Let $M^{\prime}$ be the
...
\begin{statement}
\textit{(i)} If $\lambda$ is an inward shifted $n$-staircase, P(s_{\lambda} \longmapsto 0) = 1-\prod_{i=1}^{n} \left(  1-\dfrac{1}{q^{i}}\right)
\end{statement}
\begin{statement}
\textit{(ii)} If $\lambda$ is an outward shifted $n$-staircase, P(s_{\lambda} \longmapsto 0) = 1-\prod_{i=1}^{n+1} \left(  1-\dfrac{1}{q^{i}}\right)
\end{statement}
